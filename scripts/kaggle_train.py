...
def map_label(label_str):
    ...
    if "ddos" in s or "mirai" in s: return "DDoS"
    if "dos" in s: return "DoS" if "ddos" not in s else "DDoS"
    if "recon" in s or "scan" in s or "sweep" in s or "discovery" in s: return "Reconnaissance"
    if "mitm" in s: return "MITM"
    if "spoof" in s or "arp" in s or "malformed" in s or "injection" in s or "xss" in s or "bruteforce" in s or "hijack" in s or "malware" in s: return "Spoofing"
    return np.nan
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if actual_len < self.seq_len:
            pad_width = self.seq_len - actual_len
            window_x = np.pad(window_x, ((0, pad_width), (0, 0)), mode='edge')
...
def main():
    ...
    criterion_expert = FocalLoss(gamma=2.0)
    ...
    t_ddos = get_roc_threshold(np.array(probs_dict["ddos"]), (val_y == label_mapping.get("DDoS")).astype(int), 0.001)
    ...
    def apply_expert_inference(p_s1, p_ddos, p_mitm, p_spoof, p_dos, p_recon, raw_x):
        batch_preds = []
        if iforest is not None:
            batch_X_pooled = raw_x[:, :, :raw_dim].mean(axis=1)
            iforest_preds = iforest.predict(batch_X_pooled)
        else:
            iforest_preds = np.full(len(p_s1), 1)

        for i in range(len(p_s1)):
            if iforest_preds[i] == -1:
                p_s1[i] = max(p_s1[i], 0.80)
            ...
            # Expert Race
            claims = []
            if p_ddos[i] > t_ddos: claims.append((label_mapping.get("DDoS"), p_ddos[i]))
            if p_mitm[i] > t_mitm: claims.append((label_mapping.get("MITM"), p_mitm[i]))
            if p_spoof[i] > t_spoof: claims.append((label_mapping.get("Spoofing"), p_spoof[i]))
            if p_dos[i] > t_dos: claims.append((label_mapping.get("DoS"), p_dos[i]))
            if p_recon[i] > t_recon: claims.append((label_mapping.get("Reconnaissance"), p_recon[i]))
            ...
            if not claims:
                ...
                top_claim = claims[0]
                if len(claims) > 1 and (top_claim[1] - claims[1][1] < CONFIG["margin_rejection"]):
                    batch_preds.append(benign_idx)
                else:
                    batch_preds.append(top_claim[0])
        return batch_preds
    ...
    with torch.no_grad():
        for batch_X, _ in test_loader:
            ...
            batch_preds = apply_expert_inference(p_s1, p_ddos, p_mitm, p_spoof, p_dos, p_recon, batch_X.numpy())
            fp32_preds.extend(batch_preds)
            
    logger.info(f"FP32 Test Preds distribution: {Counter(fp32_preds)}")

    # --- ONNX Export ---
    logger.info("Applying Export and Dynamic Quantization...")
